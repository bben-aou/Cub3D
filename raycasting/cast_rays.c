...
/*   Updated: 2022/11/09 18:37:51 by bben-aou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../cub3d.h"

void    facingUpDown(t_var *var)
{
...
void    getIntersectionVertical(t_var *var)
{
    ...
    if (var->ray->horizontalWallFound == 1)
        var->ray->horizDistance = getHorizDistance(var);
    else
        var->ray->horizDistance = LONG_MAX;
    if (var->ray->verticalWallFound == 1)
        var->ray->verticDistance = getVerticDistance(var);
    else
